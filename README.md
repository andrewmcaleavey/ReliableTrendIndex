...
I think the RTI is a demonstrably better operationalization of reliable
change than the RCI in almost any case with more than 2 data points.
With two data points, it simplifies to the RCI. The chief added
assumption with more time points is that linear change is a reasonable
approximation of the true change or that a linear trend is of interest
for the data regardless of any curvilinear aspects. So, the RTI assumes
you are interested in an overall tendency to increase or decrease over
...
In clinical and psychological measurement, we often ask whether an
observed *change* in scores is larger than what could be expected from
measurement error. The **Reliable Change Index (RCI; Jacobson & Truax,
1991)** addresses this for a **two-timepoint difference score**.

However, in many contexts we observe a *series* of measurements from a
single person (e.g., weekly or daily assessments). In such cases, it is
natural to ask whether there is a **reliable linear trend** over time,
not just a reliable change between two occasions.

The **Reliable Trend Index (RTI)** extends the logic of the RCI to this
setting: it is a standardized test statistic for the within-person slope
in a regression of scores on time, using a reliability-adjusted estimate
of the slope’s standard error.

------------------------------------------------------------------------

### Regression framework

Suppose one person has $n$ observations $y_1,\dots,y_n$ at equally
spaced times $t=1,\dots,n$. We fit a simple regression model
...
If the measurement has reliability $r$ and single-occasion standard
deviation $SD$, then the standard deviation of measurement error is
...
The **Reliable Trend Index (RTI)** is defined as the slope estimate
divided by this reliability-adjusted standard error:

$$
\text{RTI} = \frac{\hat\beta_1}{SD \sqrt{1-r}\,\sqrt{\tfrac{12}{n(n^2-1)}}}.
$$

This statistic can be interpreted analogously to a $t$-test: values
beyond $\pm 1.96$ (approx.) suggest a trend unlikely to be explained by
measurement error alone, at the $\alpha=0.05$ level.

------------------------------------------------------------------------

### Relationship to the Reliable Change Index (RCI)

When $n=2$, the regression slope reduces to the difference between the
two observations:
...
The RCI is thus a special case of the RTI for two timepoints. The RTI
generalizes the logic to an arbitrary number of repeated measures,
testing whether a *trend* in scores is reliably greater than chance,
given the measure’s reliability.
...
The RTI is defined most basically as $$RT = \frac{b}{s_{e}}$$ Here, $b$
is the linear time effect (slope) estimate from a regression of all
available data points and $s_{e}$ is the appropriate standard error for
that slope, estimated from external data and using the same assumptions
as the RCI. However, $s_{e}$ will be smaller than $S_{diff}$ as a
function of the number of observations. The logic of this is that the
more observations of a series, the less likely a random series is to
have a notably increasing or decreasing linear trend so the plausible
error decreases.

There are currently two operationalizations of RTI definitions. The
first version of this package used
[`metafor`](https://cran.r-project.org/package=metafor) as a backend,
which is a flexible approach. The second uses a simpler dependency-free
approach to extend the RCI to multiple time points, but lacks some
flexibility. Current plan is to use the simpler model for most
functions, but this is not implemented yet. The text below only applies
to the {`metafor`} approach.
