...
Jacobson & Truax defined the RCI as:
...
While both the RCI and RTI are similar, there are two main differences.
First, the RCI includes only two observations while the RTI includes any
number of observations per person. Second, though they can be based on
the same data (*i.e.*, a reliability estimate and population standard
deviation), the standard error terms are different. The difference is
that, $s_{e}$ will be smaller than $S_{diff}$ as a function of the
number of observations (with two observations, they are equal). The
logic of this is that the more observations in a series, the less likely
