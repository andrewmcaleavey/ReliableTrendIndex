---
title: "Historical RTI workflows"
output:
...
vignette: >
  %\VignetteIndexEntry{Historical RTI workflows}
...
This article preserves older workflows for reproducibility. For new analyses,
use `rti()` for one series, `rci()` for two-occasion change, and `rti_by()`
for long-format grouped data. See `vignette("migration")` for replacements.

The functions illustrated below remain functional but are deprecated and may
emit warnings. They are planned to remain available until at least version 1.0.0.
...
As of August 2025, I am in the process of adding more basic functions that implement the simplest versions of RTI using base R. This should be easier to use for many purposes. Full support is not available yet. Functions in this family include `slope_se_reliability()` and `rti_by_person()`. These are not fully developed yet and are still untested.
...
sim_data_2 <- simulated_data |>
  ...
  purrr::map(\(df) rti(values = df$obs_score, sem = 0.2)) |>
  # take the output and simplify it to data.frame-friendly columns
  purrr::map_dfr(rti_to_df) |>
  # re-add id (one per group, ordered like split())
  dplyr::mutate(
    id = simulated_data |>
      dplyr::group_by(id) |>
  ...
  dplyr::right_join(simulated_data, by = "id") |>
  # compute first/last score per person and keep last row
  dplyr::group_by(id) |>
  dplyr::mutate(
    first_obs = dplyr::first(obs_score),
    last_obs  = dplyr::last(obs_score)
  ) |>
  dplyr::slice_tail(n = 1)
