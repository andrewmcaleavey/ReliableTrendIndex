...
#'
#' @param x data
#' @param divideByTwo logical.
#' @return A table
#' @export
#' @examples
#' relChangePct_table(generate_data())
relChangePct_table <- function(x, divideByTwo = TRUE){
  denom <- if (divideByTwo) length(unique(x$id)) else nrow(x)
  100 * relChangeRaw_table(x, divideByTwo = divideByTwo) / denom
}
