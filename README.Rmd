...
Note that it gave us this message on screen: `"More than two values provided, assuming they are evenly spaced in time."`. The function `rti_calc_simple()` is meant to be *simple*, so it is not meant to be used with uneven assessment spacing. Other methods allow more flexibility at the cost of increase complexity. 
...
Because of this, I recommend using the `summary()` function when possible. 
...
So now we can clearly see that the RCI, if ignoring the interim measurements, would not detect a reliable change, but the RTI would. 
...
That will give the full information. However, note that the re-analysis changed `mac_rti$values` to deviation-from-first-observation scores rather than raw scores (and inserted a leading 0 for the first observation). This should not have dire consequences but must be carefully examined, and if the observed scores are important, altering this behavior is possible.   
...
Notice that the trend line is going up, and the 95% CI for the trend does not include a flat line. Therefore the RTI suggests zero change is unlikely.  
