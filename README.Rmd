...
There are currently two operationalizations of RTI in this package. The first version of this package used [`metafor`](https://cran.r-project.org/package=metafor) as a backend, which is a flexible approach. The second uses a simpler dependency-free approach to extend the RCI to multiple time points, but lacks some flexibility. The current recommendation is to use the simpler model for most applications as it is faster and requires fewer dependencies. However, at this time, some complex examples are only available with the original implementation. The text below only applies to the {`metafor`} approach.
